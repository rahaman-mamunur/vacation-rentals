  ...
  try {
    ...
    if (existingRequest) {
      return res
        .status(409)
        .send(
          'A booking cancellation with these details already has been sent.'
        );
    }
...
router.get('/reqofcancellation', async (req, res) => {
  try {
    const reqofcancellation = await Bookingcancel.find({}).populate(
      'bookingInfo'
    );
...
router.post('/reqofcancellation-accepted', async (req, res) => {
  ...
  try {
    ...
    const bookingdeletedItem = await Booking.findOneAndDelete({
      _id: bookingid,
    });
    ...
    const store = temp.filter(
      (item) => item.bookingId.toString() !== bookingdeletedItem._id.toString()
    );
    ...
    res
      .status(201)
      .send('Booking and cancellation records deleted successfully');
  } catch (error) {
    console.log(error);
    res.status(500).send('Internal server Error');
  }
