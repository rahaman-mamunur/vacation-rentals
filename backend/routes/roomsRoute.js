...
router.post('/addroom', async (req, res) => {
  const { room, rentperday, maxcount, description, phonenumber, type, images } =
    req.body;
...
router.delete('/delete-rooms/:id', async (req, res) => {
  try {
    ...
    const temperIds = result.flatMap((item) =>
      item.currentbookings.map((item2) => item2.bookingId.toString())
    );
    ...
    const reportedItem = await Report.deleteMany({
      booking: { $in: bookingIds },
    });

    console.log('reported item length', reportedItem.length);

    const cancelItem = await Report.deleteMany({
