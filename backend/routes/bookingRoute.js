  ...
  try {
    ...
    if (payment) {
      try {
        ...
        res
          .status(201)
          .json({ message: 'Your Room has been booked succeessfully!' });
      } catch (error) {
        console.log(error);
  ...
  try {
    ...
    for (const item of bookings) {
      ...
      if (!checkInDate.isValid()) {
        return res
          .status(400)
          .json({ message: 'Invalid check-in date format' });
      }
      ...
      const updater = await Booking.findOneAndUpdate(
        ...
        {
          $set: {
            checkCancelDate: checkCancelDate,
            cancellationDeadline: checkCancelDate
...
router.post('/cancelbooking', async (req, res) => {
  ...
  try {
    ...
    if (!bookings || bookings.length === 0) {
      return res
        .status(403)
        .json({ message: 'No bookings found for the room' });
    }

    const bookingExistingInRoom = bookings.some(
    ...
    res
      .status(201)
      .json({ message: 'Your Room has been Cancelled successfully!' });
  } catch (error) {
    console.log(error);
    res.status(500).json({ message: 'Internal server error' });
  }
...
router.delete('/delete-bookings/:id', async (req, res) => {
  ...
  try {
    ...
    const cancelItem = await Bookingcancel.findOneAndDelete({
